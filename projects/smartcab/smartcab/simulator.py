...
# pylint: disable=C0103,C0111,C0301,C0326,C0330,W0150,W0212,W0703
...
class Simulator(object):
    ...
    def run(self, tolerance=0.05, n_test=0):
        ...
        while True:
            ...
            print()
            print("/-------------------------")
            print("| {} trial {}".format('Testing' if testing else 'Training', trial))
            print(r"\-------------------------")
            print()
    ...
    def render_text(self, trial, testing=False):
        ...
        if status and status['waypoint'] is not None: # Continuing the trial
            ...
            if self.env.enforce_deadline:
                curtime = (status['deadline'] - 1) * 100.0 / (status['t'] + status['deadline'])
                print("{:.0f}% of time remaining to reach destination.".format(curtime))
            else:
    ...
    def render(self, trial, testing=False):
        ...
        if status:
            ...
            if self.env.enforce_deadline:
                curtime = (status['deadline'] - 1) * 100.0 / (status['t'] + status['deadline'])
                self.screen.blit(self.font.render("{:.0f}% of time remaining to reach destination.".format(curtime), True, self.colors['black'], self.bg_color), (350, 100))
            else:
